...
const Reports: React.FC<ReportsProps> = ({ transactions, expenses, maintenanceJobs }) => {
  ...
  React.useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (period === 'daily' && selectedDate !== today) {
        // Only auto-switch if the user was looking at "today" (which is now "yesterday")
        const now = new Date();
  ...
  const stats = useMemo(() => {
    ...
    const startOfSelectedDay = new Date(y, m - 1, d, 0, 0, 0);
    const endOfSelectedDay = new Date(y, m - 1, d, 23, 59, 59);
              ...
              {stats.periodTransactions.length > 0 ? (
                stats.periodTransactions.map(t => (
                  <tr key={t.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="p-6">
                      <span className={`px-3 py-1 rounded-full text-[9px] font-black ${t.type === 'expense' ? 'bg-red-50 text-red-600' :
